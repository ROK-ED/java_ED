...
public class ProductDAOImpl extends DAO implements ProductDAO {
    ...
    private final String SELECT_INFO = "select * from baskinrobbins where now_value<= ? or (input_date + ?) <= sysdate ";
    ...
    private final int PERIOD_STANDERD = 15; // 구매요망조회 시 입고 후 기간
	...
	public List<Product> selectInfo() {
		...
		try {
    		...
    		pstmt.setInt(2, PERIOD_STANDERD);
    		rs = pstmt.executeQuery();
    		while(rs.next()) {
    			Product product = new Product();
    			product.setProductId(rs.getInt("product_id"));
